...
public class RobotsMatcher : IRobotsParseHandler
{
    ...
    private readonly byte[] _indexHtmBytes = "/index.htm"u8.ToArray();
    ...
    public void HandleAllow(int lineNum, ReadOnlySpan<byte> value)
    {
        while (true)
        {
            ...
            else
            {
                ...
                if (slashPos == -1 || !value[slashPos..].StartsWith(_indexHtmBytes)) return;
                var len = slashPos + 1;
                var newpattern = new byte[len + 1];
                value[..len].CopyTo(newpattern);
                newpattern[len] = (byte)'$';
                value = newpattern;
                continue;
            }
    ...
    // match scores.
    private class MatchHierarchy
    {
        public readonly Match Global = new(); // Match for '*'
        public readonly Match Specific = new(); // Match for queried agent.
    ...
    private readonly MatchHierarchy _allow = new(); // Characters of 'url' matching Allow.
    private readonly MatchHierarchy _disallow = new(); // Characters of 'url' matching Disallow.

    private bool _seenGlobalAgent; // True if processing global agent rules.
    private bool _seenSpecificAgent; // True if processing our specific agent.
    private bool _everSeenSpecificAgent; // True if we ever saw a block for our agent.
    private bool _seenSeparator; // True if saw any key: value pair.
